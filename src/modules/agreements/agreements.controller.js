...
class AgreementsController {
    ...
    async analyze(req, res) {
        try {
            ...
            res.status(500).json({ success: false, message: 'Failed to analyze agreement' });
        }
    }
    ...
    async extractText(req, res) {
        try {
            ...
            if (!extractedText || extractedText.trim().length === 0) {
                extractedText = "[System Warning: Empty text extracted. Ensure PDF contains selectable text, not scanned images.]";
            }
